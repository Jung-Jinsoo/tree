(function(e) {
	...
	e.Autocompleter = function(t, n) {
		function d() {
			...
			s.val(i);
			w();
			s.trigger("result", [ r.data, r.value ]);
			return true
		}
		function v(e, t) {
			if (l == r.DEL) {
		...
		function g(r) {
			...
			if (s == r.length) {
				...
				i = m(r.replace(r.substring(s), ""))
			}
			return i[i.length - 1]
		}
		function y(i, o) {
			if (n.autoFill && g(s.val()).toLowerCase() == i.toLowerCase() && l != r.BACKSPACE) {
				s.val(s.val() + o.substring(g(u).length));
				e(t).selection(u.length, u.length + o.length)
			}
		}
		function b() {
			clearTimeout(o);
		...
		function w() {
			...
			if (n.mustMatch) {
				s.search(function(e) {
					if (!e) {
						if (n.multiple) {
							var t = m(s.val()).slice(0, -1);
							s.val(t.join(n.multipleSeparator) + (t.length ? n.multipleSeparator : ""))
						} else {
							s.val("");
		...
		function S(r, s, o) {
			...
			if (u) {
				if (u.length) {
					...
					var f = n.parse && n.parse(n.noRecord) || x(n.noRecord);
					s(r, f)
				}
		...
		var i = null;
		if (n.failure != null && typeof n.failure == "function") {
			i = n.failure
		}
		var s = e(t).attr("autocomplete", "off").addClass(n.inputClass);
		var o;
		var u = "";
		var a = e.Autocompleter.Cache(n);
		var f = 0;
		var l;
		var c = {
			mouseDownOnSelect : false
		};
		var h = e.Autocompleter.Select(n, t, d, c);
		var p;
		navigator.userAgent.indexOf("Opera") != -1 && e(t.form).bind("submit.autocomplete", function() {
			if (p) {
				p = false;
				return false
			}
		});
		s.bind((navigator.userAgent.indexOf("Opera") != -1 && !"KeyboardEvent" in window ? "keypress" : "keydown") + ".autocomplete", function(t) {
			f = 1;
			l = t.keyCode;
			switch (t.keyCode) {
			case r.UP:
			...
			function n(e, n) {
				...
				if (n && n.length) {
					for ( var i = 0; i < n.length; i++) {
						if (typeof n[i].result === "undefined") {
							return
						} else {
				...
				if (typeof t == "function")
					...
					s.trigger("result", r && [ r.data, r.value ])
			}
			var t = arguments.length > 1 ? arguments[1] : null;
			e.each(m(s.val()), function(e, t) {
				S(t, n, n)
			})
		}).bind("flushCache", function() {
			a.flush()
		}).bind("setOptions", function() {
			e.extend(true, n, arguments[1]);
	...
	e.Autocompleter.Select = function(t, n, r, i) {
		function p() {
			if (!l)
				return;
			c = e("<div/>").hide().addClass(t.resultsClass).css("position", "absolute").appendTo(document.body).hover(function(t) {
		...
		function m(e) {
			if (t.scrollJumpPosition || !t.scrollJumpPosition && !(e < 0 && u == 0 || e > 0 && u == o.size() - 1)) {
				u += e;
				if (u < 0) {
					u = o.size() - 1
				} else if (u >= o.size()) {
