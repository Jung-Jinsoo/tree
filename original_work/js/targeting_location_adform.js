...
$(function() {
	$("#base_targeting").validationEngine('attach', {
		onValidationComplete: function(form, status) {
			...
			if(status) {
				...
				$('#countriesspan${param.count}', parent.document).text($('#result_locations_value', parent.document).val());
			}	
		}
	});	
	
	$('#interests').focus().autocomplete("/target/get_facebookAutocompleteSearch.htm?adsId="+$('#adsid').val()+"&accountId="+$('#adaccountid').val()+"&type=adkeyword", autocomplete_option);
	...
	$('.closebtn').live("click", function () {
		...
		if(this_ele.attr('id')=="keywords_closebtn"){
			$('span[id="keywords_key"]').each(function () {
				return_key += $(this).text() + ",";
			});
			
			interestsAppend(return_key);
			
			if($('#keywords_closebtn').size() == 0) {
				$('#suggest').hide();
			}
		}
	});	
...
function interestsAppend(data){
	var keywords = data.substring(0, data.length-1); 
	//console.log(data);
